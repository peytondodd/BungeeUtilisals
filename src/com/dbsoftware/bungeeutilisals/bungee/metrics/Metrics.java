package com.dbsoftware.bungeeutilisals.bungee.metrics;
...
public class Metrics {
	...
	public boolean isOptOut() {
		synchronized (optOutLock) {
			try {
				...
				if (debug) {
					ProxyServer.getInstance().getLogger().log(Level.INFO, "[BungeeUtilisals] " + ex.getMessage());
				}
				return true;
			}
	...
	private void postPlugin(final boolean isPing) throws IOException {
		...
		boolean onlineMode = ProxyServer.getInstance().getConfigurationAdapter().getBoolean("online_mode", true); // TRUE
																													// if
																													// online
																													// mode
																													// is
