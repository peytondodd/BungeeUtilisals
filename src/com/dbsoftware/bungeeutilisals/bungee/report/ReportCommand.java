...
public class ReportCommand extends Command {
	...
	public static void executeReportToggleCommand(CommandSender sender, String[] args) {
		...
		if (!players.contains(p)) {
			...
			p.sendMessage(TextComponent.fromLegacyText(getReports().getFile()
					.getString("Reports.Messages.EnabledAlert", "&cNew reports will now be alerted!")
					.replace("&", "§")));
		} else {
			players.remove(p);
			p.sendMessage(TextComponent.fromLegacyText(getReports().getFile()
					.getString("Reports.Messages.DisabledAlert", "&cNew reports will not be alerted anymore!")
					.replace("&", "§")));
		}
	}
	...
	public static void executeReportDeleteCommand(CommandSender sender, String[] args) {
		...
		if (!ReportAPI.getReportNumbers().contains(number)) {
			p.sendMessage(
					TextComponent.fromLegacyText(getReports().getFile()
							.getString("Reports.Messages.NoReportNumberExist",
									"&cReport number %number% doesn't exist!")
							.replace("&", "§").replace("%number%", number + "")));
			return;
		}
		ReportAPI.removeReport(number);
		p.sendMessage(TextComponent.fromLegacyText(getReports().getFile()
				.getString("Reports.Messages.Removed", "&cYou have removed report number %number%!").replace("&", "§")
	...
	public static void executeReportCommand(CommandSender sender, String[] args) {
		...
		if (args.length < 2) {
			p.sendMessage(TextComponent.fromLegacyText(getReports().getFile()
					.getString("Reports.Messages.WrongArgs", "&cPlease use /report (player) (reason)")
					.replace("&", "§")));
			return;
		} else if (args.length == 1) {
			if (args[0].contains("toggle")) {
				if (BungeeUtilisals.getInstance().getConfig().getBoolean("Bukkit-Permissions")) {
					PluginMessageChannel.sendPermissionCheckPluginMessage("hasPermission", "butilisals.report.toggle",
							"report", args, ((ProxiedPlayer) sender));
					return;
				}

				if (!p.hasPermission("butilisals.report.toggle") && !p.hasPermission("butilisals.*")
						&& !getReports().getFile().getStringList("Players").contains(p.getName())) {
					p.sendMessage(
							TextComponent.fromLegacyText(getReports().getFile()
		...
		if (args[0].contains("remove") || args[0].contains("delete")) {
			if (BungeeUtilisals.getInstance().getConfig().getBoolean("Bukkit-Permissions")) {
				PluginMessageChannel.sendPermissionCheckPluginMessage("hasPermission", "butilisals.report.remove",
						"report", args, ((ProxiedPlayer) sender));
				return;
			}
		...
		if (System.currentTimeMillis() - lastUsed < cdmillis) {
			int timeLeft = (int) (getReports().getFile().getInt("Cooldown", 60)
					- (System.currentTimeMillis() - lastUsed) / 1000L);
			p.sendMessage(TextComponent.fromLegacyText(getReports().getFile()
					.getString("Reports.Messages.CooldownMessage",
							"&cYou need to wait &6%time% &cseconds untill you can report again!")
					.replace("&", "§").replace("%time%", timeLeft + "")));
			return;
		}
		...
		if (reported == null) {
			p.sendMessage(TextComponent.fromLegacyText(getReports().getFile()
					.getString("Reports.Messages.NoPlayer", "&cYou can't report an offline player!")
					.replace("&", "§")));
		...
		p.sendMessage(TextComponent.fromLegacyText(
				getReports().getFile().getString("Reports.Messages.Reported", "&aYou have reported %player%!")
						.replace("&", "§").replace("%player%", reported.getName())));
		for (ProxiedPlayer pl : players) {
			if (pl != null) {
				pl.sendMessage(TextComponent.fromLegacyText(getReports().getFile()
						.getString("Reports.Messages.ReportBroadcast",
								"&a%player% &ehas been reported by &a%reporter% &efor &a%reason%&e!")
						.replace("&", "§").replace("%player%", reported.getName()).replace("%reporter%", p.getName())
	...
	public void execute(CommandSender sender, String[] args) {
		...
		if (BungeeUtilisals.getInstance().getConfig().getBoolean("Bukkit-Permissions")) {
			PluginMessageChannel.sendPermissionCheckPluginMessage("hasPermission", "butilisals.report", "report", args,
