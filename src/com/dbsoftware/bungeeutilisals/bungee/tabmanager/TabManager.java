...
public class TabManager {
    ...
    public static void loadTab(){
        ...
        if(!tab.getFile().getKeys().contains("Tab")){
	        ...
	        tab.getFile().set("Tab.Header", Arrays.asList("&6This server uses", "&eThis server uses", "&aThis server uses", "&2This server uses", "&6&lBungeeUtilisals", 
	        		"&6You are on %server%", "&eYou are on %server%", "&aYou are on %server%", "&2You are on %server%"));
	        tab.getFile().set("Tab.Footer", Arrays.asList("&6This server uses", "&4This server uses", "&cThis server uses", "&bThis server uses", "&3&lBungeeUtilisals",
	        		"&6Online players: %globalcount%", "&4Online players: %globalcount%", "&cOnline players: %globalcount%", "&bOnline players: %globalcount%"));
	        tab.save();
        }
