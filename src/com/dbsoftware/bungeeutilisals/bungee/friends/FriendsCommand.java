...
public class FriendsCommand	extends Command {
	...
	public static void executeFriendAddCommand(CommandSender sender, String[] args, int limit){
		if(limit == 0){
			limit = Friends.friends.getInt("Friends.Limit.Limit", 10) + 1;
		}
		if(sender.getName().toLowerCase().equals(args[1].toLowerCase())){
			sender.sendMessage(Utils.format(Friends.friends.getString("Friends.Messages.Error", "&cYou can't add yourself as a friend.")));
			return;
		}
    	...
    	if(myfriends.size() >= limit && Friends.friends.getBoolean("Friends.Limit.Enabled", true)){
			sender.sendMessage(Utils.format(Friends.friends.getString("Friends.Messages.ReachedLimit", "").replace("&", "§").replace("%player%", args[1])));
    		return;
    	}
		...
		if(p == null){
            ...
            if(friends.contains(args[1])){
				sender.sendMessage(Utils.format(Friends.friends.getString("Friends.Messages.AlreadyFriend", "").replace("&", "§").replace("%player%", sender.getName())));
				return;
            }
			
            FriendsAPI.addFriendRequest((ProxiedPlayer)sender, args[1]);
            sender.sendMessage(Utils.format(Friends.friends.getString("Friends.Messages.RequestSent", "").replace("&", "§").replace("%player%", sender.getName())));
			return;
		} else {
      ...
      if(!BungeeUtilisals.getDatabaseManager().isConnected()){
		...
		if(args.length == 0){
			...
			if(args[0].contains("list")){
				...
				sender.sendMessage(Utils.format(Friends.friends.getString("Friends.Messages.FriendListFormat.Header", "&a&m&l-------&r&c&l Friends &a&m&l-------").replace("&", "§")));
				for(String s : friends){
					...
					if(friend != null){
						...
						sender.sendMessage(Utils.format(Friends.friends.getString("Friends.Messages.FriendListFormat.OfflineFormat", "&7- &a%friend% &7is offline!").replace("&", "§").replace("%friend%", s)));
					}
				}
				sender.sendMessage(Utils.format(Friends.friends.getString("Friends.Messages.FriendListFormat.Footer", "&a&m&l-------&r&c&l Friends &a&m&l-------").replace("&", "§")));
			} if(args[0].contains("requests")){
				ProxiedPlayer p = (ProxiedPlayer)sender;
				List<String> requests = FriendsAPI.getFriendRequests(p.getName());
				
				sender.sendMessage(Utils.format(Friends.friends.getString("Friends.Messages.RequestListFormat.Header", "").replace("&", "§")));
				...
				sender.sendMessage(Utils.format(Friends.friends.getString("Friends.Messages.FriendListFormat.Footer", "").replace("&", "§")));
			...
			if(args[0].contains("add")){
				...
				if(p == null){
		            ...
		            if(requests.contains(args[1])){
		            	...
		            	sender.sendMessage(Utils.format(Friends.friends.getString("Friends.Messages.Accepted", "").replace("&", "§").replace("%player%", args[1])));
		            }
	            	return;
				} else {
		            List<String> requests = FriendsAPI.getFriendRequests(sender.getName());
		            ...
		            if(requests.contains(args[1])){
		            	...
		            	sender.sendMessage(Utils.format(Friends.friends.getString("Friends.Messages.Accepted", "").replace("&", "§").replace("%player%", p.getName())));
		            	p.sendMessage(Utils.format(Friends.friends.getString("Friends.Messages.AcceptedFriend", "").replace("&", "§").replace("%player%", sender.getName())));
		            }
				}
				...
				if(p == null){
		            ...
		            if(requests.contains(args[1])){
		            	...
		            	sender.sendMessage(Utils.format(Friends.friends.getString("Friends.Messages.Denied", "").replace("&", "§").replace("%player%", args[1])));
		            }
	            	return;
				} else {
		            List<String> requests = FriendsAPI.getFriendRequests(sender.getName());
		            ...
		            if(requests.contains(args[1])){
		            	...
		            	sender.sendMessage(Utils.format(Friends.friends.getString("Friends.Messages.Denied", "").replace("&", "§").replace("%player%", p.getName())));
		            	p.sendMessage(Utils.format(Friends.friends.getString("Friends.Messages.DeniedFriend", "").replace("&", "§").replace("%player%", sender.getName())));
		            }
				}
				...
				if(p == null){
					if(!FriendsAPI.getFriends(sender.getName()).contains(args[1])){
						sender.sendMessage(Utils.format(Friends.friends.getString("Friends.Messages.RemoveError", "&cYou can't remove someone who is not your friend.").replace("&", "§")));
						return;
					}
					FriendsAPI.removeFriend((ProxiedPlayer)sender, args[1]);
					sender.sendMessage(Utils.format(Friends.friends.getString("Friends.Messages.Removed", "").replace("&", "§").replace("%friend%", args[1])));
					return;
				} else {
					...
					sender.sendMessage(Utils.format(Friends.friends.getString("Friends.Messages.Removed", "").replace("&", "§").replace("%friend%", args[1])));
					p.sendMessage(Utils.format(Friends.friends.getString("Friends.Messages.Remove", "").replace("&", "§").replace("%player%", sender.getName())));
				}
			...
			if(args[0].contains("msg")){
				ProxiedPlayer p = ProxyServer.getInstance().getPlayer(args[1]);
				if(p == null){
					sender.sendMessage(Utils.format(Friends.friends.getString("Friends.Messages.NoPlayer", "").replace("&", "§")));
					return;
				} else {
					List<String> friends = FriendsAPI.getFriends(sender.getName());
					if(!friends.contains(p.getName())){
						sender.sendMessage(Utils.format(Friends.friends.getString("Friends.Messages.NoFriendMsgError", "").replace("&", "§")));
						return;
					}
					if(sender == p){
						sender.sendMessage(Utils.format(Friends.friends.getString("Friends.Messages.MsgError", "").replace("&", "§")));
						return;
					} else {
				          String message = "";
						...
						sender.sendMessage(Utils.format(Friends.friends.getString("Friends.Messages.MsgSend", "").replace("&", "§").replace("%player%", p.getName()).replace("%message%", message)));
						p.sendMessage(Utils.format(Friends.friends.getString("Friends.Messages.MsgReceive", "").replace("&", "§").replace("%player%", sender.getName()).replace("%message%", message)));
			...
			if(args[0].contains("reply")){
				if(!Friends.msging.containsKey((ProxiedPlayer)sender)){
					sender.sendMessage(Utils.format(Friends.friends.getString("Friends.Messages.NoReply", "").replace("&", "§").replace("%player%", sender.getName())));
					return;
				}
