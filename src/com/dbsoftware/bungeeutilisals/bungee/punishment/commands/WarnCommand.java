...
public class WarnCommand extends Command {
	...
	public static void executeWarnCommand(CommandSender sender, String[] args) {
		String warnreason = "";
		if(args.length < 2){
			for(String s : Punishments.punishments.getStringList("Punishments.Warn.Messages.WrongArgs", Arrays.asList(new String[]{"&4&lBans &8» &ePlease use &b/warn (player) (reason) &e!"}))){
		...
		if(p != null){
			...
			warnreason = reason;
			for(String s : Punishments.punishments.getStringList("Punishments.Warn.Messages.WarnMessage", Arrays.asList(new String[]{
			...
			for(String s : Punishments.punishments.getStringList("Punishments.Warn.Messages.OfflinePlayer", Arrays.asList(new String[]{"&4&lBans &8» &eThat player is not online!"}))){
				sender.sendMessage(TextComponent.fromLegacyText(s.replace("&", "§")));
			}
			return;
		}
		...
		pinfo.addWarn();
		for(String s : Punishments.punishments.getStringList("Punishments.Mute.Messages.Warned", Arrays.asList(new String[]{"&4&lBans &8» &b%player% &ehas been Warned! &bReason: &e%reason%"}))){
